...
**已实现**：
- CRD：`Tool` 新增 `Builtin *BuiltinTool{Names []BuiltinToolName}`（v1alpha2），
  enum 校验 + 与 McpServer/Agent 的 CEL 互斥校验，deepcopy 与 CRD manifest 已重新生成并同步到 helm chart；
- Translator：`compiler.go` 把 builtin 名字去重后写入 `AgentConfig.BuiltinTools`；
  `manifest_builder.go` 中 builtin 含 bash 时启用与 skills 相同的代码执行隔离（privileged/srt 沙箱）；
- Go 运行时：新增 `tools.NewWorkspaceTools(dir, names)`（复用 skills 工具实现、自动建目录、
  过滤未知名字、不含 skills 发现工具），`buildAgentTools` 挂载 CRD 请求的 builtin 工具并与
  skills 工具去重；无 skills 时工作区默认 `/tmp/kagent-workspace`；
- UI：ChatToolsPanel 显示/增删 Built-in 工具（修复了 groupMcpToolsByServer 丢弃非 MCP 工具的问题），
  "Add tools" 里可一键添加 bash/file 工具；TS 类型同步；
- 测试：translator 单测（翻译 + 去重 + bash 隔离）、运行时单测（过滤/建目录/空输入）。
...
- ✅ 空状态：显示 Agent 名称 + 描述（`spec.description` 经 ChatAgentProvider 下发）+ 快捷键/附件提示；
- ✅ 会话侧栏时间分组：调研发现已存在（`GroupedChats.tsx`，today/yesterday/older），无需重做；
- 待做：长会话消息虚拟化（`react-virtuoso` 或分页加载历史 task）；侧栏搜索/过滤；
  流式输出中断后的"继续/重试"按钮。
...
| 第 5 批 | A1 Builtin 工具 CRD 字段 + 运行时挂载（E2E 留待 CI） | 1-2 周，Go 为主 | ✅ 第一期已实现 |
...
对整个分支 diff 做了 8 角度审查（正确性 3 + 清理 3 + 架构 1 + 规范 1）。已修复的确认问题：

| 问题 | 位置 | 修复 |
|------|------|------|
| 预览对话框不再过滤 `style` 属性（安全回归，CSS url() 可远程 beacon） | DocumentPreviewDialog.tsx | 重新剥离 `style` |
| 工具面板保存丢失 `headersFrom` 等字段、跨 namespace 同名服务器被合并（数据损坏） | ChatToolsPanel.tsx | 写路径改用原始 Tool 条目，不再 groupMcpToolsByServer |
| 删除 Agent 工具按名字匹配、忽略 namespace（误删同名工具） | ChatToolsPanel.tsx | 改用完整 ref 匹配 |
| 纯附件消息携带空文本块（部分 provider 报 400） | ChatInterface.tsx | 无文本时不加 text part |
| 紧凑工具行把 TokenStatsTooltip 套在按钮里（嵌套 button，非法 HTML） | ToolDisplay.tsx | 移出行按钮 |
| 切换会话后 follow-scroll 状态未重置（新会话停在顶部） | ChatInterface.tsx | 初始化时重置 isNearBottomRef |
| CJK 输入法在 Safari 上回车误发送 | ChatInterface.tsx | 增加 keyCode 229 保护 |
| CodeBlock 每次渲染都递归提取文本、预览对话框常驻挂载（流式卡顿） | CodeBlock.tsx | useMemo 缓存 + 仅打开时挂载 |
| 附件 base64 长期驻留 React state（内存膨胀） | ChatInterface.tsx | 显示副本剥离 bytes |

**已核实为误报**：Go 端上传落盘"只在配 skills 时生效"——executor 的 skillsDirectory 默认 `/skills` 从不为空，`GetSessionPath` 无条件创建 uploads/、symlink 为 best-effort，故 builtin-only agent 的上传落盘正常工作。

**已做的清理（第 8 批）**：
- `updateAgentModelConfig`/`updateAgentTools` 合并为共享 `updateDeclarativeAgentSpec`（读-改-写只此一处，未来加 resourceVersion 冲突保护落一处即可）；
- 抽出共享 `AttachmentChip`（composer + 已发送消息）与 `CopyButton`（CodeBlock + ToolDisplay）组件；
- Python `add_builtin_tools_to_agent` / `add_skills_tool_to_agent` 共用 `_append_workspace_tools` 工厂映射。

**未处理（记为后续清理，非阻断）**：`updateDeclarativeAgentSpec` 仍无 resourceVersion 冲突保护（并发编辑可能互相覆盖）；Go-Python 文件名清洗逻辑跨语言重复（有意保留，各自语言实现）；A1 的 E2E 测试待 CI 补。
